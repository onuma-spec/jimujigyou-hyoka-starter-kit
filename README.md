...
住民が「続行・廃止・見直し」を判断できる、単一HTMLの評価ツールができあがります。対象規模の目安として、これまで7自治体（142件〜652件の事業数）で実績があります。導入自治体一覧・実際に動いているものは本体リポジトリを参照してください。
...
データ抽出・AI要約生成の技術詳細は「ビルドの流れ」を参照してください。
