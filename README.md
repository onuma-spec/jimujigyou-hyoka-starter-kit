...
1. 👤 AIコーディング支援（Claude Code等）を任意のフォルダで開き、「このリポジトリ（`https://github.com/onuma-spec/jimujigyou-hyoka-starter-kit`）をcloneして、スターターキット.mdを読んで、〇〇市を追加したい」と伝える。gitが使えなくてもAIがZIPを直接取得して展開できるので、事前のダウンロード作業は不要
