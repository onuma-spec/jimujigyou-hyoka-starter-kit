...
**注意**：`build_XX.py`側の`LABELS.show_outcome`は、データの有無ではなく利用者（制作者）の好みで
`False`にできる（fixed_prompt.md参照。カードの情報量を減らしたい場合等）。利用者が「成果セクションは不要」と
判断した場合は、本ファイルによる`story_p2`のAI生成パス自体を実行不要（API呼び出しの節約）。
ただし省略する前に必ず利用者へ確認すること（fixed_prompt.md参照）。
...
出力ルール：
- story_p1：事業の目的と実施内容を1〜2文（40〜80字）で。「〜のために〜しています。」の流れで自然な日本語にする。
...
**story_p1**
- [ ] 概要の単なる言い換えになっていないか（目的→実施内容の流れがあるか）
